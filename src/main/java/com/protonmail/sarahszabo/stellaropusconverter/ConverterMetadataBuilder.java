...
public class ConverterMetadataBuilder {
     ...
     */
    public ConverterMetadataBuilder(Map<StellarOPUSConverter.MetadataType, String> map) {
        this.artist = Objects.requireNonNull(map.get(StellarOPUSConverter.MetadataType.ARTIST));
        this.title = Objects.requireNonNull(map.get(StellarOPUSConverter.MetadataType.TITLE));
        if (map.get(StellarOPUSConverter.MetadataType.CREATED_BY) != null) {
            this.createdBy = map.get(StellarOPUSConverter.MetadataType.CREATED_BY);
        }
        if (map.get(StellarOPUSConverter.MetadataType.DATE) != null) {
            this.date = LocalDate.parse(map.get(StellarOPUSConverter.MetadataType.DATE), StellarOPUSConverter.DATE_FORMATTER);
        }
        if (map.get(StellarOPUSConverter.MetadataType.ALBUM_ART) != null) {
            this.albumArtPath = StellarGravitonField.newPath(map.get(StellarOPUSConverter.MetadataType.ALBUM_ART));
        }
    }
    ...
    public ConverterMetadataBuilder addAll(ConverterMetadata metadata) {
        this.artist = StellarGravitonField.preferredTitleFormat(Objects.requireNonNull(metadata.getArtist()));
        this.title = StellarGravitonField.preferredTitleFormat(Objects.requireNonNull(metadata.getTitle()));
        if (metadata.getCreatedBy() != null && !StellarOPUSConverter.isDefaultMetadata(StellarOPUSConverter.MetadataType.CREATED_BY, metadata)) {
            this.createdBy = StellarGravitonField.preferredTitleFormat(metadata.getCreatedBy());
        }
        if (metadata.getDate() != null && !StellarOPUSConverter.isDefaultMetadata(StellarOPUSConverter.MetadataType.DATE, metadata)) {
            this.date = metadata.getDate();
        }
        if (metadata.getAlbumArtPath() != null && !StellarOPUSConverter.isDefaultMetadata(StellarOPUSConverter.MetadataType.ALBUM_ART, metadata)) {
     ...
     * @return This builder, per <i>the builder pattern</i>
     */
    public ConverterMetadataBuilder addAll(Map<StellarOPUSConverter.MetadataType, String> map) {
        this.artist = StellarGravitonField.preferredTitleFormat(Objects.requireNonNull(map.get(StellarOPUSConverter.MetadataType.ARTIST)));
        this.title = StellarGravitonField.preferredTitleFormat(Objects.requireNonNull(map.get(StellarOPUSConverter.MetadataType.TITLE)));
        if (map.get(StellarOPUSConverter.MetadataType.CREATED_BY) != null) {
            this.createdBy = StellarGravitonField.preferredTitleFormat(map.get(StellarOPUSConverter.MetadataType.CREATED_BY));
        }
        if (map.get(StellarOPUSConverter.MetadataType.DATE) != null) {
            this.date = LocalDate.parse(map.get(StellarOPUSConverter.MetadataType.DATE), StellarOPUSConverter.DATE_FORMATTER);
        }
